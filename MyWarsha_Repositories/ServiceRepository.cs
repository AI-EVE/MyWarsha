...
namespace MyWarsha_Repositories
{
    public class ServiceRepository : Repository<Service>, IServiceRepository
    {
        ...
        public async Task<IEnumerable<ServiceDto>> GetAll(PaginationPropreties paginationPropreties, Expression<Func<Service, bool>> predicate, decimal? minPrice, decimal? maxPrice)
        {
            ...
            {
                query = query.Where(s => s.ServiceFees.Sum(sf => sf.TotalPriceAfterDiscount) + s.ProductsToSell.Sum(ps => ps.TotalPriceAfterDiscount) >= minPrice);
            }

            if (maxPrice.HasValue)
            {
                query = query.Where(s => s.ServiceFees.Sum(sf => sf.TotalPriceAfterDiscount) + s.ProductsToSell.Sum(ps => ps.TotalPriceAfterDiscount) <= maxPrice);
            }
